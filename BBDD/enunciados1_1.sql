...
select idpartida,  
sum(case	
	when puntos_final - puntos_inicio > 0 then 1
    when puntos_final - puntos_inicio < 0 then 0
	else 0
end) as rondas_ganadas, es_banca
from turnos
where es_banca=1
group by idpartida;
...
select idpartida, round(avg(apuesta),2) as apuesta_media
from turnos t1
group by idpartida;
